...
from monitor.daily_summary import build_daily_summary, format_summary_text
...
def run():
    ...
    try:
        ...
        print(f'[{date_str}] Daily summary built for {summary["date"]} '
              f'— {len(summary["rows"])} cycle(s), {summary["grand_total"]} min total')
        ...
        Event.objects.create(
            ...
            message=f'Daily summary sent for {summary["date"]} '
                    f'({summary["grand_total"]} min total outage)'
        )
